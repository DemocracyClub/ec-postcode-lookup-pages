...
def test_sopn_day(template_sorter, election_date_template_sorter):
    ...
    single_election_date_template_sorter = election_date_template_sorter(single_ballot_sorter_before_deadline, single_ballot_sorter_before_deadline.dates[0])
    ...
    assert single_election_date_template_sorter.sections[0].mode == None
	# BallotSection	
    assert single_election_date_template_sorter.sections[1].weight == 0
    assert single_election_date_template_sorter.sections[1].mode == None
    ...
    first_election_date = multiple_ballot_template_sorter_before_deadline.dates[0]
    first_election_date_template_sorter = election_date_template_sorter(multiple_ballot_template_sorter_before_deadline, first_election_date)
    assert len(first_election_date_template_sorter.sections) == 2
    assert first_election_date_template_sorter.sections[0].weight == 0
    assert first_election_date_template_sorter.sections[0].mode == None
    assert first_election_date_template_sorter.sections[1].weight == 0
    assert first_election_date_template_sorter.sections[1].mode == None

    second_election_date = multiple_ballot_template_sorter_before_deadline.dates[1]
    second_election_date_template_sorter = election_date_template_sorter(multiple_ballot_template_sorter_before_deadline, second_election_date) 
    ...
    assert second_election_date_template_sorter.sections[0].mode == "List of candidates published"
    assert second_election_date_template_sorter.sections[1].weight == 0
    assert second_election_date_template_sorter.sections[1].mode == "List of candidates published"
    ...
    single_election_date_template_sorter = election_date_template_sorter(single_ballot_sorter_after_deadline, single_ballot_sorter_after_deadline.dates[0])
    ...
    first_election_date = multiple_ballot_template_sorter_after_deadline.dates[0]
    first_election_date_template_sorter = election_date_template_sorter(multiple_ballot_template_sorter_before_deadline, first_election_date)
    assert len(first_election_date_template_sorter.sections) == 2
    assert first_election_date_template_sorter.sections[0].weight == 0
    assert first_election_date_template_sorter.sections[0].mode == None
    ...
    second_election_date = multiple_ballot_template_sorter_after_deadline.dates[1]
    second_election_date_template_sorter = election_date_template_sorter(multiple_ballot_template_sorter_before_deadline, second_election_date) 
    assert len(second_election_date_template_sorter.sections) == 2
    assert second_election_date_template_sorter.sections[0].weight == 0
    assert second_election_date_template_sorter.sections[0].mode == "List of candidates published"
    ...
    with pytest.raises(Exception):
        template_sorter(
            NO_LOCAL_BALLOTS, date=datetime.date(2024, 4, 6)
        )
...
def test_registration_deadline(
    ...
    assert single_election_date_template_sorter.sections[0].weight == 0
    assert single_election_date_template_sorter.sections[0].mode == "Register to vote deadline"
    assert single_election_date_template_sorter.sections[1].weight == 0
    assert single_election_date_template_sorter.sections[1].mode == "Register to vote deadline"
    ...
    assert len(second_election_date_template_sorter.sections) == 2
    assert second_election_date_template_sorter.sections[0].weight == 0
    assert second_election_date_template_sorter.sections[0].mode == 'Register to vote deadline'
    assert second_election_date_template_sorter.sections[1].weight == 0
    assert second_election_date_template_sorter.sections[1].mode == 'Register to vote deadline'
    ...
    single_election_date_template_sorter = election_date_template_sorter(single_ballot_sorter_after_deadline, single_ballot_sorter_after_deadline.dates[0])
    assert single_election_date_template_sorter.sections[0].weight == 0
    assert single_election_date_template_sorter.sections[0].mode == "Postal vote application deadline" 
    assert single_election_date_template_sorter.sections[1].weight == 0 
    assert single_election_date_template_sorter.sections[1].mode == "Postal vote application deadline"
    ...
    first_election_date = multiple_ballot_template_sorter_after_deadline.dates[0]
    first_election_date_template_sorter = election_date_template_sorter(multiple_ballot_template_sorter_after_deadline, first_election_date)
    assert len(first_election_date_template_sorter.sections) == 2
    assert first_election_date_template_sorter.sections[0].weight == 0
    assert first_election_date_template_sorter.sections[0].mode == None
    assert first_election_date_template_sorter.sections[1].weight == 0
    ...
    second_election_date = multiple_ballot_template_sorter_after_deadline.dates[1]
    second_election_date_template_sorter = election_date_template_sorter(multiple_ballot_template_sorter_after_deadline, second_election_date) 
    assert len(second_election_date_template_sorter.sections) == 2
    assert second_election_date_template_sorter.sections[0].weight == 0
    assert second_election_date_template_sorter.sections[0].mode == "Postal vote application deadline"
    assert second_election_date_template_sorter.sections[1].weight == 0
    assert second_election_date_template_sorter.sections[1].mode == "Postal vote application deadline"


    with pytest.raises(Exception):
        template_sorter(
            NO_LOCAL_BALLOTS, date=datetime.date(2024, 4, 17)
        )
...
def test_postal_vote_application_deadline(
    ...
    with pytest.raises(Exception):
        template_sorter(
            NO_LOCAL_BALLOTS, date=datetime.date(2024, 4, 18)
        )
...
def test_vac_application_deadline(
    ...
    single_election_date_template_sorter = election_date_template_sorter(single_ballot_sorter_after_deadline, single_ballot_sorter_after_deadline.dates[0])
    assert single_election_date_template_sorter.sections[0].weight == 0
    assert single_election_date_template_sorter.sections[0].mode == "VAC application deadline" 
    assert single_election_date_template_sorter.sections[1].weight == 0 
    assert single_election_date_template_sorter.sections[1].mode == "VAC application deadline"
    ...
    assert len(second_election_date_template_sorter.sections) == 2
    assert second_election_date_template_sorter.sections[0].weight == 0
    assert second_election_date_template_sorter.sections[0].mode == 'VAC application deadline'
    assert second_election_date_template_sorter.sections[1].weight == 0
    assert second_election_date_template_sorter.sections[1].mode == 'VAC application deadline'


    with pytest.raises(Exception):
        template_sorter(
            NO_LOCAL_BALLOTS, date=datetime.date(2024, 4, 25)
        )
